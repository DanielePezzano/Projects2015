...
namespace BLL.Engine.Planet.Production
{
    public class OreUpdater : ProductionUpdater, IProcutionUpdater, IUpdater
    {
        ...
        {
            foreach (var detail in ReferredPlanetDto.Buildings.Where(c => c.BuildingType == BuildingType.Civil).SelectMany(building => building.Details.Where(c => c.Bonus == BonusType.OreBonus)))
            {
        ...
        {
            foreach (var bonus in TechnologyDtos.Where(c => c.Field == "Buildings" && c.Field != "ShipComponent" && c.Field != "ShipFrame" && c.Field != "Weapons").SelectMany(technologyDto => technologyDto.TechnologyBonuses.Where(c => c.Bonus == BonusType.OreBonus)))
            {
        ...
        public void Update()
        {
            if (ConsistencyCheckOre.ConsistencyCheck == false || Product < 1) return;
            UpdateToDo = true;
